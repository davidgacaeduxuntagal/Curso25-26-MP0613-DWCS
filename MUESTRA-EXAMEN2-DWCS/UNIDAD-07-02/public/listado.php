...
use Clases\{Producto, Tienda};
use Philo\Blade\Blade;
use Jaxon\Jaxon;
use function Jaxon\jaxon; 

// La clase Prestaciones 
class Prestaciones {
    public function listarTiendas($idProducto)  {
        $respuesta = jaxon()->newResponse();
        $tienda = new Tienda();
        $tiendas = $tienda->buscarTiendas($idProducto);
        $texto = 'Tiendas con el producto: ' . PHP_EOL;
        foreach ($tiendas as $tienda) {
            $texto .= $tienda->nombre . ' - ' . $tienda->tlf . ' - ' . $tienda->unidades . PHP_EOL;
        }       
...
$jaxon->register(Jaxon::CALLABLE_CLASS, Prestaciones::class);
