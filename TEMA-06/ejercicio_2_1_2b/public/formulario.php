...
// Obtener divisas y nombre completo de la divisa
$solicitud  = new ISOCodesDetailed();
$codigosISOXML    = $servicio->ISOCodesDetailed($solicitud)
                           ->getISOCodesDetailedResult()
                           ->getAny();

//    Eliminamos el esquema XML, sino no podemos convertirlo a SimpleXMLElement
$codigosISOXML = preg_replace('/<xs:schema.*<\/xs:schema>/s', '', $codigosISOXML);

//    Parseamos el XML
$codigosISOSimpleXML = new SimpleXMLElement($codigosISOXML);

//   Creamos un array con la información
$infoCodigosISO = [];
foreach ($codigosISOSimpleXML->DocumentElement[0]->ISOCodes as $iso) {
    $infoCodigosISO[(string)$iso->ISO] = (string) $iso->DescriptionEN;
}
